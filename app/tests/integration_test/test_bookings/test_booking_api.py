...
async def test_add_and_get_booking(room_id, date_from, date_to, status_code, booked_rooms, authentificated_ac: AsyncClient):
    response = await authentificated_ac.post("/bookings", params={
        "room_id": room_id, 
        "date_from": date_from, 
        "date_to": date_to
    })
    ...
    response = await authentificated_ac.get("/bookings")
...
async def test_get_and_delete_all_bookings(authentificated_ac: AsyncClient):
    ...
    for booking in response.json():
        response = await authentificated_ac.delete(f"/bookings/{booking['id']}")
